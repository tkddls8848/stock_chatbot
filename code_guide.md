...
> **Claude Code는 이 파일을 자동으로 읽지 않는다.** 예전 `CLAUDE.md`는 세션마다
> 자동 로드됐지만 이 파일은 아니다. 작업을 시작할 때 이 문서를 먼저 열어야
> 아래 규칙이 적용된다.
...
- **문서는 이 기준 문서, 절차서 하나, 배포서 하나, 그리고 계획서뿐이다.
  그 외 새 목록 파일을 만들지 않는다.** `infra/server-ops.md`는 이미 떠 있는 서버를 상대로 반복하는
