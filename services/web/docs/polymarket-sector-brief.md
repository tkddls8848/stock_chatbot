...
> 완료 기록은 git 이력이 맡는다.
>
> `services/web/docs/polymarket-dashboard.md`(현재 스냅숏 대시보드)를 **전제로** 한다. 그
...
`services/web/polymarket/dashboard/taxonomy.py`에 둔다. 태그 지식은 이미 그 모듈이
...
프롬프트 파일은 `services/web/prompts/polymarket_brief_ko.txt`(그룹)와
`services/web/prompts/polymarket_brief_overview_ko.txt`(종합) 둘이다.
...
`data/webpub/polymarket/sector_brief.json`. `services/web/core/storage.py`의 원자적 쓰기만
쓴다.
...
독립 one-shot `services/web/polymarket/sector_brief.py`. 봇 프로세스와 무관하다.
...
`services/web/core/config.py`의 리터럴 상수. 운영자가 조정하는 값이 아니므로 env가 아니다.
...
서버에 변경된 `services/web/llm/polymarket_brief.py`, `services/web/polymarket/sector_brief.py`,
`services/web/prompts/polymarket_brief_ko.txt`를 배포한 뒤
