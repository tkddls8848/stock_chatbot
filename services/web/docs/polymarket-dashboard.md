...
```text
브라우저 → Caddy(TLS·인증 정책) → 127.0.0.1:8788 → services/web/server.py
                                                ↓
                                      data/webpub/*
```
...
그러나 이 저장소의 `services/telegram_bot/features/market_sentiment/polymarket.py`에는
`/markets/keyset`이 어떤 cursor 이름을 넘겨도 첫 페이지를 반복했고 실제 limit도
100에서 잘렸다는 운영 실측이 있다. 현재 `services/web/tests/test_polymarket_smoke.py`도 함수명과
...
`services/web/tests/test_polymarket_smoke.py`에 opt-in 프로브를 추가하고 실제 운영 Lightsail에서
다음 명령으로 실행한다.
...
**확률 순위에서는 마감이 `POLYMARKET_MIN_HOURS_TO_END`(72시간) 안인 event를 뺀다.**
5분짜리 가격 방향이나 오늘 밤 경기의 확률 0.99는 컨센서스가 굳은 것이 아니라 결과가
곧 나오는 것이다. 빼지 않으면 “가장 굳은 예측” 여덟 칸이 마감 시계를 재는 표가
된다. 그날 트렌드(`trending.candidates`)가 후보를 고를 때와 같은 상수를 읽는다.
...
```text
services/web/polymarket/dashboard/
  __init__.py
...
services/web/polymarket/refresh.py
services/web/polymarket/repository.py
...
services/web/tests/test_webpub.py  페이지·API 회귀
services/web/tests/test_polymarket_smoke.py  events keyset·offset 실제 프로브
```

`code_guide.md`의 현재 Polymarket 승격, `/polymarket` 명령, 90일 이력,
`polymarket_rules.py` allowlist 설명은 6단계 완료 시점의 사실과 충돌하므로 함께
교체한다. 계획서 목록도 `services/web/docs/polymarket-dashboard.md`로 바꾼다.

### 10-3. 제거·이동할 코드와 설정

새 웹의 4b와 독립 배포가 검증된 뒤 제거한다.

- `services/telegram_bot/features/market_sentiment/handlers.py`의 `/polymarket` command·gate·caption
- `services/telegram_bot/handlers/navigation.py`의 폴리마켓 메뉴·callback
- `services/telegram_bot/features/market_sentiment/snapshot.py`의 08:35 job
- `services/telegram_bot/features/market_sentiment/feature.py`의 client/store 주입과 scheduler wiring
- `services/telegram_bot/features/market_sentiment/chart.py`의 `render_polymarket_chart`
- `services/telegram_bot/features/market_sentiment/polymarket_rules.py`
- `services/telegram_bot/features/market_sentiment/polymarket_history.py`
- `app/polymarket_backfill.py`
- `services/telegram_bot/state/polymarket_consensus.py`와 `services/telegram_bot/state/__init__.py` export
- 기존 client를 옮긴 뒤 `services/telegram_bot/features/market_sentiment/polymarket.py`
- proxy factory를 옮긴 뒤 `services/telegram_bot/features/market_sentiment/polymarket_proxy.py`
