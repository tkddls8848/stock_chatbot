"""현재 Polymarket 컨센서스 화면(`/polymarket`)."""
...
@media(max-width:520px){.pm-controls{grid-template-columns:1fr}.pm-event{grid-template-columns:1fr}.pm-prob{text-align:left}.pm-bar{grid-template-columns:100px 1fr}.pm-bar-value{grid-column:2;text-align:left}.links{overflow-x:auto}.navin{padding-right:8px}.brand{display:none}}
    ...
    + h1(I_BARS, "폴리마켓 예측 현황")
    + "<div class='disc'>지금 폴리마켓에 열려 있는 예측 질문 전체를 분야별로 정리한 현재 시점 기록입니다. "
    "확률은 참여자들이 건 가격에서 나온 값이며 <b>사실 보증이나 매매 신호가 아닙니다.</b> "
...
<section class='histbox' aria-labelledby='pm-explorer-title'><div class='histh' id='pm-explorer-title'><span class='phico'>"""
 ...
 <form class='pm-controls' id='pm-controls' role='search'>
  ...
  <select id='pm-sort' name='sort' aria-label='정렬'><option value=''>기본(검색하면 관련도순)</option><option value='relevance'>관련도</option><option value='volume24hr'>24시간 거래량</option><option value='liquidity'>유동성</option><option value='leader_probability'>1위 확률</option><option value='end_date'>종료일</option><option value='title'>제목</option></select>
...
_POLYMARKET_SCRIPT = (
    "<script>" + JS_UTIL + """
const PM_STATUS={ok:'정상',low_liquidity:'낮은 유동성',no_liquidity:'유동성 0',liquidity_missing:'유동성 정보 없음',unavailable:'확률 읽기 불가'};
...
function renderSummary(d){pmGeneration=d.generation_id;const a=d.accounting||{},x=d.activity||{};document.getElementById('pm-events').textContent=(a.open_event_count||0).toLocaleString();document.getElementById('pm-markets').textContent=(x.market_count||0).toLocaleString();document.getElementById('pm-volume').textContent=money(x.volume24hr);document.getElementById('pm-liquidity').textContent=money(x.liquidity);document.getElementById('pm-category-cover').textContent=prob(d.named_category_ratio);document.getElementById('pm-price-cover').textContent=prob(a.open_event_count?(a.consensus_ready_event_count/a.open_event_count):0);
 const fresh=d.freshness||{},alert=document.getElementById('pm-alert');alert.className='pm-alert '+(['delayed','stale'].includes(fresh.state)?'warn':'');alert.textContent='기준 '+stamp(d.generated_at)+' (한국 시간) · '+(d.coverage_status==='complete'?'전체 수집 완료':'일부만 수집됨')+' · '+(PM_FRESH[fresh.state]||'갱신 상태 미상')+' · 순위는 데이터가 정상인 질문만 셉니다.';document.getElementById('asof-date').textContent='폴리마켓 '+stamp(d.generated_at)+' (한국 시간)';
...
async function loadBrief(){const m=document.getElementById('pm-brief-meta'),b=document.getElementById('pm-brief');const r=await fetch('/api/polymarket/sector-brief');if(!r.ok){m.textContent='아직 컨센서스 정리가 없습니다.';b.innerHTML='';return}const d=await r.json();
 m.textContent='기준 '+stamp(d.written_at)+' (한국 시간)'+(d.generation_id!==pmGeneration&&pmGeneration?' · 직전 수집분 기준입니다':'')+' · 집계는 전체, 인용은 거래량 상위 '+(d.named_limit||0)+'건';
...
function trendSide(items,empty){if(!items||!items.length)return "<p class='empty'>"+empty+"</p>";return items.map(r=>"<button type='button' class='pm-rank' data-event='"+esc(r.id)+"'><b>"+esc(r.title)+"</b><small>현재 "+prob(r.probability)+" · 24시간 "+money(r.volume24hr)+(r.volume_change!=null?" · 거래량 증감 "+money(r.volume_change):'')+"</small></button>").join('')}
async function loadTrending(){const m=document.getElementById('pm-trend-meta'),b=document.getElementById('pm-trend');const r=await fetch('/api/polymarket/trending');if(!r.ok){m.textContent='아직 트렌드 집계가 없습니다.';b.innerHTML='';return}const d=await r.json();
 ...
 m.textContent=(d.state==='warming_up'?'오늘 기준선을 막 세웠습니다 — 다음 주기부터 이동을 조명합니다':basis+' 이동 · 후보 '+Number(d.candidate_count||0).toLocaleString()+'건(24시간 거래량 '+money(d.min_volume)+' 이상)')+(pmGeneration&&d.generation_id!==pmGeneration?' · 직전 수집분 기준입니다':'');
