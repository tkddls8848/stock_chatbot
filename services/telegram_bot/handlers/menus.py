...
def admin_menu(registry) -> InlineKeyboardMarkup:
    rows = [[("시스템 상태", "nav:system:show")]]
    if registry.is_enabled("instruments"):
        rows[0].append(("종목 DB 갱신", "nav:stockdb"))
    return _keyboard([*rows, *_back()])
