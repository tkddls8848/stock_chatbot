...
from services.telegram_bot.features.news_prefilter.service import NewsPrefilter, _rank_auc
...
def service_pending_limit():
    from services.telegram_bot.features.news_prefilter.service import _PENDING_CANDIDATE_LIMIT
...
def test_optimizer_resumes_trials_and_rests_until_new_labels(monkeypatch):
    from services.telegram_bot.features.news_prefilter import optimizer
...
def test_constant_score_average_precision_equals_prevalence():
    from services.telegram_bot.features.news_prefilter.optimizer import _average_precision
...
def test_model_save_failure_keeps_current_model(tmp_path, monkeypatch):
    import pytest
    from services.telegram_bot.features.news_prefilter import service as module
    from services.telegram_bot.features.news_prefilter.optimizer import OptimizationResult
