...
from services.telegram_bot.core import storage
from services.telegram_bot.core.storage import write_json_atomic
from services.telegram_bot.state.market_digest import MarketDigestStore
...
def test_stock_db_build_keeps_the_previous_cache_when_saving_fails(
    ...
    from services.telegram_bot.stocks.database import StockDatabase
    ...
    monkeypatch.setattr("services.telegram_bot.stocks.database._fetch_a_code_name", lambda: a_shares)
    monkeypatch.setattr("services.telegram_bot.stocks.database._fetch_hk_spot", lambda: None)
    monkeypatch.setattr("services.telegram_bot.stocks.database.fetch_kr_listed", lambda: [])
    monkeypatch.setattr("services.telegram_bot.stocks.database.fetch_us_listed", lambda: [])
    _break_replace(monkeypatch)
...
def test_manual_briefing_reports_a_failed_send_to_the_caller(monkeypatch):
    ...
    from services.telegram_bot.briefing import service
...
def test_scheduled_briefing_contains_the_same_failure(caplog):
    ...
    from services.telegram_bot.features.briefing.feature import _run_scheduled
