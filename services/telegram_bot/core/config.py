...
# ── 시장상황 보고서 ──────────────────────────────────
# 매시간 원문만 수집하고 UTC +9 00·04·08·12·16·20시에 **발행할지부터 판정한다.**
# 4시간은 검토 주기이고 발행 주기가 아니다 — 재료가 얇거나 직전 보고서의
...
# 3시간에서 4시간으로 늘렸다(운영자 결정, 2026-09-24). 24의 약수여야 cron `*/N`이
# 자정을 넘을 때 간격이 어긋나지 않는다.
NEWS_REPORT_INTERVAL_HOURS = 4
...
# 한 구간이 담는 총량. 주기를 3→4시간으로 늘리며 20% 올렸다(600→720, 2026-09-24).
# 소스당 상한(12)은 매시간 수집 한 번의 몫이라 주기와 무관해 그대로 둔다.
NEWS_REPORT_QUEUE_MAX_ITEMS = 720
# 시장 하나의 보고서에 넣을 헤드라인 수와, 근거로 뽑아 보여줄 건수.
# 헤드라인 144건이면 입력이 약 7,200토큰이라 출력 예약 4,096을 더해도 컨텍스트
# 32,768의 35% 선이다. 올릴 때는 이 계산을 다시 한다. 주기를 3→4시간으로
# 늘리며 20% 올렸다(120→144, 2026-09-24).
NEWS_REPORT_MAX_HEADLINES = 144
