...
# 그쪽은 remote_coding/remote-lightsail 의 verify-host.sh 가 소유한다.
...
check "운영 .env 소유권·권한" bash -c \
    "[ \"\$(sudo stat -c '%U:%G %a' '$APP_DIR/.env')\" = '$APP_USER:$APP_USER 600' ]"
...
check "최근 24시간 내 백업 산출물" bash -c \
    "sudo find /var/backups/stock-chatbot -maxdepth 1 -name 'backup-*.tgz' -mtime -1 | grep -q ."

# 서버 타임존이 UTC 이므로 03:00 KST/JST 는 크론에서 18:00 이어야 한다.
# 타임존을 JST 로 착각해 '0 3' 로 적으면 정오에 돌아 자동 스냅샷 직전이 아니게 된다.
check "백업 크론 시각이 UTC 기준" bash -c \
    "sudo grep -Eq '^0 18 \* \* \* ' /etc/cron.d/stock-chatbot-backup"
