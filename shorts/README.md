...
영상은 인트로, 선정한 개별 이슈 최대 5개, 고지문 순서입니다. 인트로는 오늘의 첫 질문을 띄우는 훅이고(예전의 "선정한 개별 이슈 2" 큰 숫자 카드는 계속 볼 이유가 되지 못했습니다), 마무리는 마무리 멘트와 같은 말을 하는 짧은 고지 두 줄입니다. 약한 분야를 억지로 채우지 않습니다. 완성과 음성 보존이 길이보다 우선입니다. `SHORTS_MAX_DURATION_SECONDS`는 기본 180초의 참고 목표이며, 초과해도 자동 배속이나 생성 중단을 하지 않습니다. 기본 발화 속도는 +0%입니다. FFmpeg와 HyperFrames 모두 전체 원고를 한 번에 합성한 단일 음성을 사용하고 끝에 0.6초 여유를 둡니다. 화면은 edge-tts가 보고한 단어별 발화 시각에 맞춰 장면별로 전환하며, 장면이 바뀌는 자리에는 1.15~1.8초의 호흡을 둡니다(자리마다 다릅니다). 플랫폼의 Shorts 분류 조건과 제작 목표는 별개이므로 긴 완성본은 게시 전 확인합니다.
...
**화면과 음성의 역할을 나눕니다.** 확률의 정확한 값(`99.95%`)은 화면에 남기고,
소리로는 듣는 사람 기준으로 풉니다("사실상 굳어진 분위기입니다"). 2026-09-23
산출물의 내레이션이 "9월 WTI 90달러 이하: 예 99.95%, 아니오 0.05%"를 그대로
낭독했는데, 소수점 둘째 자리와 예·아니오 쌍은 표를 눈으로 읽는 문법이지 말의
문법이 아닙니다. 규칙은 `src/polymarket_shorts/speech.py`에 있고 **LLM을 부르지
않습니다** — 무료 한도가 떨어진 날에도 원고의 자연스러움이 같아야 합니다.
49.5 대 50.5는 "정확히 반반", 22%는 "다섯 번에 한 번꼴"입니다. 두 선택지의 말이
같으면 한 문장으로 합쳐 같은 문장을 두 번 듣지 않게 합니다.
...
**정지 카드는 셋으로 움직입니다.** 큰 수치가 0에서 제자리까지 차오르고(0.72초,
여덟 칸. 올라가는 동안에는 정수만 보여 주고 확정된 값은 마지막에 한 번 제대로
섭니다), 선택지 줄이 말의 순서대로 한 줄씩 쌓이며(빈 게이지 홈은 처음부터 그려
두어 이미 뜬 줄이 밀리지 않습니다), 프레임 전체가
아주 느리게 흐릅니다(가장자리 8px을 잘라 내고 그 안에서 ±7px, 주기 23·31초).
셋 다 기존 FFmpeg·Pillow 경로 안에서 만들고 새 입력·의존성·네트워크가 없습니다.
되돌려 키우는 비율이 1.5%라 글자가 무뎌지지 않고, 자막은 그 뒤에 얹으므로
흔들리지 않습니다. 도입·마무리도 제목을 먼저 세운 뒤 문구를 얹습니다.
...
옆(y=677)으로 올라갔습니다.
자막 줄바꿈은 렌더가 어절 경계에서 넣고 libass는 그대로 그립니다(`WrapStyle=2`).
...
클립이 없으면 기존 정지 배경 렌더 경로와 FFmpeg 명령을 그대로 사용합니다.
...
- `backgrounds/<이슈 해시>.mp4`를 캐시합니다. 도입은 첫 이슈 클립을 공유하고 마무리는
  정지 배경입니다. 자연어 수정본은 원본 폴더의 캐시를 재사용합니다.
- 클립이 있으면 비트별 중간 인코딩 후 이어 붙이고 자막을 넣습니다. 정지 구간의
  드리프트는 중간 인코딩에서만 적용합니다. 음악과 업로드 자동화는 추가하지 않습니다.
