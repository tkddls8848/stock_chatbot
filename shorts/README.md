...
영상은 인트로, 선정한 개별 이슈 최대 5개, 고지문 순서입니다. 약한 분야를 억지로 채우지 않습니다. 완성과 음성 보존이 길이보다 우선입니다. `SHORTS_MAX_DURATION_SECONDS`는 기본 180초의 참고 목표이며, 초과해도 자동 배속이나 생성 중단을 하지 않습니다. 기본 발화 속도는 +0%입니다. FFmpeg와 HyperFrames 모두 전체 원고를 한 번에 합성한 단일 음성을 사용하고 끝에 0.6초 여유를 둡니다. 화면은 edge-tts가 보고한 단어별 발화 시각에 맞춰 장면별로 전환하며, 장면이 바뀌는 자리에는 1.15~1.8초의 호흡을 둡니다(자리마다 다릅니다). 플랫폼의 Shorts 분류 조건과 제작 목표는 별개이므로 긴 완성본은 게시 전 확인합니다.
...
있습니다. 장면별 확인점(`watch_point`)은 화면의 체크포인트로만 남기고, 고지문은
마무리에서 한 번만 말합니다. 장면마다 "…확인하세요"를 붙이면 같은 당부를 다섯 번
...
섭니다), 화면 문구의 선택지 줄이 말의 순서대로 한 줄씩 쌓이며, 프레임 전체가
